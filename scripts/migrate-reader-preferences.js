...
if (!process.argv.includes('--apply'))
  throw new Error('Ejecuta este script con --apply para confirmar la migración');
...
async function run() {
  ...
  try {
    ...
    assignments.forEach(assignment => {
      if (assignment.readerId && assignedMassesByReader.has(assignment.readerId))
        assignedMassesByReader.get(assignment.readerId).add(assignment.massId);
      (assignment.substituteIds || []).forEach(readerId =>
        assignedMassesByReader.get(readerId)?.add(assignment.massId),
      );
    });
    ...
    const updates = readers.map((reader, index) => {
      ...
      const unavailableMassIds =
        unavailableCandidates.length && crypto.randomInt(100) < 35 ? [randomItem(unavailableCandidates)] : [];
    ...
    console.log(
      `Preferencias generadas para ${readers.length} lectores ficticios en ${masses.length} misas activas.`,
    );
  } finally {
    await client.close();
  }
