...
function renderAvailabilityReport() {
  const content = $('#availabilityReport');
  if (!content) return;
  ...
  const unassigned = state.readers
    ...
    .sort((a, b) => a.name.localeCompare(b.name, 'es', { sensitivity: 'base' }));
  const substituteSections = masses.length
    ? masses
        .map(mass => {
          const readers = [...substitutesByMass.get(mass.id)].sort((a, b) =>
            readerName(a).localeCompare(readerName(b), 'es', { sensitivity: 'base' }),
          );
          return `<section class="availability-report-group"><div class="availability-report-group-head"><h4>${esc(mass.name)}</h4><span>${esc(massSchedule(mass))}</span></div><ol>${readers.map(readerId => `<li>${esc(readerName(readerId))}</li>`).join('')}</ol></section>`;
        })
        .join('')
    : '<p class="availability-report-empty">No hay suplentes asignados durante este mes.</p>';
  const unassignedRows = unassigned.length
    ? unassigned
        .map(reader => {
          const preferred = activeMasses.filter(mass => readerPrefersMass(reader, mass.id));
          return `<tr><td><b>${esc(reader.name)}</b><small>${reader.substituteOnly ? 'Solo suplente' : 'Lector normal'}</small></td><td>${preferred.length ? preferred.map(mass => esc(mass.name)).join(' · ') : 'Ninguna misa'}</td></tr>`;
        })
        .join('')
    : '<tr><td colspan="2">Todos los lectores activos tienen una asignación este mes.</td></tr>';
  content.innerHTML = `<div class="availability-report-heading"><p class="eyebrow">PARROQUIA · MINISTERIO DE LECTORES</p><h2>Suplentes y lectores disponibles</h2><p>${esc(monthLabel(state.month))} · Este documento no incluye lectores titulares.</p></div><section class="availability-report-section"><div class="availability-report-section-head"><h3>Suplentes asignados</h3><span>${substituteIds.size}</span></div><div class="availability-report-groups">${substituteSections}</div></section><section class="availability-report-section"><div class="availability-report-section-head"><h3>Lectores sin asignación</h3><span>${unassigned.length}</span></div><table><thead><tr><th>Lector</th><th>Misas preferidas</th></tr></thead><tbody>${unassignedRows}</tbody></table></section><p class="availability-report-footer">Generado desde el planificador de lectores.</p>`;
}

function printAvailabilityReport() {
  const report = $('#availabilityReport');
  if (!report) return toast('No se pudo preparar el reporte', true);
  const target = document.createElement('div');
  target.id = 'availabilityPrintReport';
  target.className = 'availability-report';
  target.innerHTML = report.innerHTML;
