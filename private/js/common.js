...
function renderReaders() {
  $('#readerList').innerHTML = state.readers.length ? state.readers.map(r => { const available=readerMassNames(r); return `<article class="card"><div class="card-top"><span class="avatar">${esc(r.name.split(/\s+/).map(x=>x[0]).slice(0,2).join('').toUpperCase())}</span><div><h3>${esc(r.name)}</h3><span class="badge ${r.active?'':'off'}">${r.active?'Activo':'Inactivo'}</span></div></div>${isAdmin?`<p>${esc(r.phone || 'Sin teléfono')}</p>`:''}<p class="availability-copy"><b>Disponible:</b> ${available.length?available.map(esc).join(' · '):'Sin horarios seleccionados'}</p>${r.notes?`<p>${esc(r.notes)}</p>`:''}<div class="reader-password-action"><button class="small-btn change-reader-password" data-id="${r.id}" data-name="${esc(r.name)}">Cambiar contraseña</button></div><div class="card-actions"><button class="small-btn edit-reader" data-id="${r.id}">Editar</button><button class="small-btn danger delete-reader" data-id="${r.id}">Eliminar</button></div></article>`; }).join('') : emptyCard('No hay lectores todavía','Agrega la primera persona del equipo.');
}
...
function renderAssignments() {
  ...
  $('#assignmentBoard').innerHTML = masses.length ? masses.map(m => {
    const availableReaders=state.readers.filter(r=>r.active && (r.availability||[]).includes(m.id));
    ...
    const datePlans=dates.map(date=>{
      ...
      const substituteOptions=state.readers.filter(reader => reader.active && !titularIds.has(reader.id) &&
        !state.assignments.some(a => a.month === state.month &&
          (a.readerId === reader.id || (a.massId !== m.id && (a.substituteIds || []).includes(reader.id)))));
      ...
      const roles=m.roles.map(role => { const a=assignment(m.id,role,date); return `<div class="role-row"><label>${esc(role)}</label><select class="assign-select" data-mass="${m.id}" data-role="${esc(role)}" data-date="${date}"><option value="">— Sin asignar —</option>${availableReaders.map(r=>`<option value="${r.id}" ${a?.readerId===r.id?'selected':''}>${esc(r.name)}</option>`).join('')}</select></div>`; }).join('');
...
document.addEventListener('click', async e => {
  ...
  if (!isAdmin && e.target.closest('#newReader,#newMass,#randomAssign,.edit-reader,.edit-mass,.delete-reader,.delete-mass')) return;
...
function openReader(r={}) { const f=$('#readerForm'), x=f.elements; f.reset(); x.id.value=r.id||'';x.name.value=r.name||'';x.phone.value=r.phone||'';x.notes.value=r.notes||'';x.active.checked=r.active!==false;$('#readerAvailability').innerHTML=state.masses.filter(m=>m.active).map(m=>`<label class="check"><input type="checkbox" name="availability" value="${m.id}" ${(r.availability||[]).includes(m.id)?'checked':''}> ${esc(massSchedule(m))}</label>`).join('')||'<span class="hint">Primero agrega una misa activa.</span>';$('#readerDialogTitle').textContent=r.id?'Editar lector':'Nuevo lector';$('#readerDialog').showModal(); }
