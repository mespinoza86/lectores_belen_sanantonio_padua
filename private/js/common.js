...
function renderReaders() {
  const filterSelect=$('#readerListFilter'),clearFilter=$('#clearReaderListFilter');
  if(readerListFilter&&!state.readers.some(reader=>reader.id===readerListFilter))readerListFilter='';
  const sortedReaders=[...state.readers].sort((a,b)=>a.name.localeCompare(b.name,'es',{sensitivity:'base'}));
  if(filterSelect)filterSelect.innerHTML=`<option value="">— Todos los lectores —</option>${sortedReaders.map(reader=>`<option value="${esc(reader.id)}" ${reader.id===readerListFilter?'selected':''}>${esc(reader.name)}</option>`).join('')}`;
...
function clearFormMessage(form){form?.querySelector('.form-message')?.remove()}
function clearFormError(form){clearFormMessage(form)}
function showFormMessage(form,message,isError=false){
  ...
  if(actions)actions.before(error);else form.append(error);
}
function showFormError(form,message){showFormMessage(form,message,true)}
...
function openSelfEditAccess(id,name){
  ...
  if(!dialog){
    ...
    $('#selfEditAccessForm').addEventListener('submit',async event=>{
      ...
      }catch(error){showFormError(form,error.message);form.elements.password.select()}
    });
  }
  const form=$('#selfEditAccessForm');form.reset();clearFormError(form);form.elements.id.value=id;$('#selfEditReaderName').textContent=name;dialog.showModal();form.elements.password.focus();
}
...
document.addEventListener('click', async e => {
  ...
  if(passwordButton){const form=$('#readerPasswordForm');form.reset();clearFormError(form);form.elements.id.value=passwordButton.dataset.id;$('#readerPasswordName').textContent=passwordButton.dataset.name;$('#readerPasswordDialog').showModal();return}
  ...
  if(e.target.closest('#copyTemporaryPassword')){const input=$('#temporaryPasswordValue'),form=input.closest('form');try{await navigator.clipboard.writeText(input.value);showFormMessage(form,'Contraseña copiada')}catch{input.select();showFormMessage(form,'Contraseña seleccionada para copiar')}return}
...
$('#readerForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,x=f.elements,id=x.id.value,preferences=$$('input[data-mass-preference]:checked',f),preferredMassIds=preferences.filter(input=>input.value==='preferred').map(input=>input.dataset.massPreference),unavailableMassIds=preferences.filter(input=>input.value==='unavailable').map(input=>input.dataset.massPreference),payload={name:x.name.value,phone:x.phone.value,notes:x.notes.value,preferredMassIds,unavailableMassIds,active:x.active.checked,substituteOnly:x.substituteOnly.checked};clearFormMessage(f);try{let result;if(selfEditingReader&&selfEditingReader.id===id){result=await request(`/api/readers/${id}/profile`,{method:'POST',body:JSON.stringify({password:selfEditingReader.password,profile:payload})});selfEditingReader=null}else result=await request(`/api/readers${id?'/'+id:''}`,{method:id?'PUT':'POST',body:JSON.stringify(payload)});f.closest('dialog').close();if(!id&&result.temporaryPassword)showTemporaryPassword(payload.name,result.temporaryPassword);else toast('Lector guardado');await load()}catch(x){showFormError(f,x.message)}});
$('#readerPasswordForm').addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget,x=f.elements;clearFormError(f);if(x.newPassword.value!==x.confirmPassword.value){showFormError(f,'Las contraseñas nuevas no coinciden');return}try{await request(`/api/readers/${x.id.value}/password`,{method:'POST',body:JSON.stringify({currentPassword:x.currentPassword.value,newPassword:x.newPassword.value,confirmPassword:x.confirmPassword.value})});f.closest('dialog').close();f.reset();toast('Contraseña actualizada')}catch(error){showFormError(f,error.message);x.currentPassword.select()}});
$('#acceptConfirmationAction').addEventListener('click',()=>{if(!pendingConfirmation)return;$('#confirmationActionDialog').close();const form=$('#confirmationPasswordForm');form.reset();clearFormError(form);$('#confirmationPasswordTitle').textContent=pendingConfirmation.action==='confirm'?'Confirmar asistencia':'Indicar que no puedes asistir';$('#confirmationPasswordDialog').showModal();form.elements.password.focus()});
$('#confirmationPasswordForm').addEventListener('submit',async e=>{e.preventDefault();if(!pendingConfirmation)return;const f=e.currentTarget,{id,action}=pendingConfirmation;clearFormError(f);try{await request(`/api/confirmations/${id}`,{method:'POST',body:JSON.stringify({action,password:f.elements.password.value})});f.closest('dialog').close();f.reset();pendingConfirmation=null;toast(action==='confirm'?'Asistencia confirmada':'Se asignó el siguiente suplente');await load()}catch(error){showFormError(f,error.message);f.elements.password.select()}});
function openEucharistReport(massId,date){const mass=state.masses.find(item=>item.id===massId);if(!mass||!hasMassEnded(date,mass.time))return toast('El reporte estará disponible una hora después del inicio de la misa',true);const assignedReader=role=>state.assignments.find(item=>item.massId===mass.id&&item.role===role&&item.date===date)?.readerId,isMonitorRole=role=>/monici|monitor/i.test(role.normalize('NFD').replace(/[\u0300-\u036f]/g,'')),monitorRole=mass.roles.find(isMonitorRole),reportRoles=mass.roles.filter(role=>!isMonitorRole(role));const form=$('#eucharistReportForm');form.reset();clearFormMessage(form);form.elements.massId.value=mass.id;form.elements.date.value=date;$('#eucharistReportHeading').textContent=mass.name;$('#eucharistReportSchedule').textContent=`${formatDate(date)} · ${mass.time}`;$('#eucharistRoleFields').innerHTML=reportRoles.map(role=>`<label>${esc(role)}<input class="eucharist-role" data-role="${esc(role)}" value="${esc(readerName(assignedReader(role)))}" required></label>`).join('');form.elements.monitor.value=monitorRole?readerName(assignedReader(monitorRole)):'';$('#eucharistReportDialog').showModal()}
...
function showReportCopiedDialog(){let dialog=$('#reportCopiedDialog');if(!dialog){dialog=document.createElement('dialog');dialog.id='reportCopiedDialog';dialog.innerHTML='<form method="dialog"><div class="dialog-head"><div><p class="eyebrow">REPORTE DE EUCARISTÍA</p><h3>Reporte copiado</h3></div></div><p>Ya puedes pegarlo en WhatsApp.</p><div class="actions"><button class="primary" value="close" autofocus>Aceptar</button></div></form>';document.body.append(dialog)}dialog.showModal()}
$('#copyEucharistReport').addEventListener('click',async()=>{const text=eucharistReportText();if(!text)return;try{await navigator.clipboard.writeText(text)}catch{const area=document.createElement('textarea');area.value=text;document.body.append(area);area.select();document.execCommand('copy');area.remove()}showReportCopiedDialog()});
...
$('#readerListFilter')?.addEventListener('change',e=>{readerListFilter=e.target.value;renderReaders()});
$('#clearReaderListFilter')?.addEventListener('click',()=>{readerListFilter='';renderReaders()});
