...
function adminReplacementSelect(assignmentId,massId,role,date){const readers=state.readers.filter(reader=>reader.active&&!reader.substituteOnly&&readerCanServeMass(reader,massId)&&!state.assignments.some(a=>a.massId===massId&&a.date===date&&a.readerId===reader.id)).sort((a,b)=>Number(readerPrefersMass(b,massId))-Number(readerPrefersMass(a,massId))||a.name.localeCompare(b.name,'es'));return `<select class="admin-replacement admin-only" data-id="${assignmentId}" data-mass="${massId}" data-role="${esc(role)}" data-date="${date}"><option value="">Asignar lector…</option>${readers.map(reader=>`<option value="${reader.id}">${esc(reader.name)}${readerPrefersMass(reader,massId)?' · Preferida':' · Disponible'}</option>`).join('')}</select>`}
...
function renderAssignments() {
  const filterSelect=$('#assignmentReaderFilter'),clearFilter=$('#clearAssignmentReaderFilter');
  if(filterSelect){
    ...
    if(clearFilter)clearFilter.disabled=!assignmentReaderFilter;
  }
  ...
  const masses = state.masses.filter(x => x.active && occurrences(x).length && (!readerMassIds||readerMassIds.has(x.id)));
  $('#assignmentBoard').innerHTML = masses.length ? masses.map(m => {
    const titularReaders=state.readers.filter(r=>r.active&&!r.substituteOnly&&readerCanServeMass(r,m.id)).sort((a,b)=>Number(readerPrefersMass(b,m.id))-Number(readerPrefersMass(a,m.id))||a.name.localeCompare(b.name,'es'));
    ...
    const datePlans=dates.map(date=>{
      ...
      const substituteOptions=state.readers.filter(reader => reader.active && readerCanServeMass(reader,m.id) && !titularIds.has(reader.id) &&
        ...
        .sort((a,b)=>Number(readerPrefersMass(b,m.id))-Number(readerPrefersMass(a,m.id))||a.name.localeCompare(b.name,'es'));
      const reserveContent=isAdmin
        ? Array.from({length:Math.max(1,reserves.length+1)},(_,index)=>`<label><span>${index+1}.</span><select class="substitute-select" data-slot="${index}"><option value="">— Sin suplente —</option>${substituteOptions.map(reader=>`<option value="${reader.id}" ${reserves[index]===reader.id?'selected':''}>${esc(reader.name)}${readerPrefersMass(reader,m.id)?' · Preferida':' · Disponible'}</option>`).join('')}</select></label>`).join('')
        : reserves.length?`<ol>${reserves.map(id=>`<li>${esc(readerName(id))}</li>`).join('')}</ol>`:'<span class="empty">Sin suplentes asignados.</span>';
      const roles=m.roles.map(role => { const a=assignment(m.id,role,date); return `<div class="role-row"><label>${esc(role)}</label><select class="assign-select" data-mass="${m.id}" data-role="${esc(role)}" data-date="${date}" data-reader="${a?.readerId||''}"><option value="">— Sin asignar —</option>${titularReaders.map(r=>`<option value="${r.id}" ${a?.readerId===r.id?'selected':''}>${esc(r.name)}${readerPrefersMass(r,m.id)?' · Preferida':' · Disponible'}</option>`).join('')}</select></div>`; }).join('');
    ...
    ? emptyCard('Sin misas para este lector',`${readerName(assignmentReaderFilter)} no participa como titular ni suplente en ${monthLabel(state.month)}.`)
...
$('#clearAssignmentReaderFilter')?.addEventListener('click',()=>{assignmentReaderFilter='';renderAssignments();if(!isAdmin)$$('.assign-select').forEach(select=>{select.disabled=true})});
