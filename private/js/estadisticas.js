...
function schedule(mass) {
  if (mass.type === 'once') {
    ...
    const date = new Date(year,month-1,day).toLocaleDateString('es-CR',{weekday:'long',day:'numeric',month:'long',year:'numeric'});
...
function coverage(preferredNormal,preferredTotal,possibleNormal,possibleTotal,roles) {
  ...
  if(preferredNormal>=roles&&preferredTotal>=minimum)return { key:'good', label:'Preferida suficiente' };
  if(possibleNormal>=roles&&possibleTotal>=minimum)return { key:'tight', label:'Requiere alternativas' };
...
function monthName(month) {
  ...
  return new Date(year,value-1,1).toLocaleDateString('es-CR',{month:'long',year:'numeric'}).replace(/^./,letter=>letter.toUpperCase());
}
...
function renderReaderConfirmations(readers, assignments, month) {
  ...
  const values=new Map(readers.map(reader=>[reader.id,{reader,confirmed:0,declined:0,pending:0}]));
  monthly.forEach(assignment=>{
    ...
    if(current){
      ...
      else if(assignment.confirmationStatus==='pending'||!assignment.confirmationStatus)current.pending+=1;
    }
  ...
  const rows=[...values.values()].filter(row=>row.confirmed||row.declined||row.pending)
    .sort((a,b)=>b.declined-a.declined||b.confirmed-a.confirmed||a.reader.name.localeCompare(b.reader.name,'es'));
  ...
  }).join('')}</tbody></table>`:`<article class="card"><h3>Sin respuestas en ${esc(monthName(month))}</h3><p>No hay confirmaciones, rechazos ni asignaciones pendientes registradas para este mes.</p></article>`;
}
...
function render(readers, masses) {
  ...
  const rows=activeMasses.map(mass=>{
    const preferred=activeReaders.filter(reader=>readerPrefersMass(reader,mass.id));
    const flexible=activeReaders.filter(reader=>readerCanServeMass(reader,mass.id)&&!readerPrefersMass(reader,mass.id));
    ...
    return { mass,preferred:preferred.length,flexible:flexible.length,unavailable,possibleTotal,status:coverage(preferredNormal,preferred.length,possibleNormal,possibleTotal,mass.roles.length) };
  }).sort((a,b)=>a.preferred-b.preferred||a.possibleTotal-b.possibleTotal||schedule(a.mass).localeCompare(schedule(b.mass),'es'));
  ...
  $('#availabilitySummary').innerHTML=`<article><span class="stat-icon green">♙</span><div><strong>${activeReaders.length}</strong><small>Lectores activos</small></div></article><article><span class="stat-icon gold">▦</span><div><strong>${activeMasses.length}</strong><small>Misas activas</small></div></article><article><span class="stat-icon rose">!</span><div><strong>${insufficient}</strong><small>Cobertura insuficiente</small></div></article><article><span class="stat-icon gold">≈</span><div><strong>${tight}</strong><small>Cobertura ajustada</small></div></article>`;
  $('#availabilityGrid').innerHTML=rows.length?rows.map(({mass,preferred,flexible,unavailable,possibleTotal,status})=>{
  ...
  }).join(''):'<article class="card"><h3>No hay misas activas</h3><p>Configura una misa para calcular su disponibilidad.</p></article>';
}
...
async function load() {
  try {
    ...
    const [readers,masses,assignments]=await Promise.all([request('/api/readers'),request('/api/masses'),request('/api/assignments')]);
    ...
    $('#availabilityGrid').innerHTML=`<article class="card"><h3>No se pudieron cargar las estadísticas</h3><p>${esc(error.message)}</p></article>`;
  }
}

$('#menuBtn').addEventListener('click',()=>$('.sidebar').classList.toggle('open'));
$('#logoutBtn').addEventListener('click',async()=>{sessionStorage.removeItem('admin_access_verified');await request('/api/auth/logout',{method:'POST'});location.replace('/')});
$('#statisticsMonth').value=currentCostaRicaMonth();
$('#statisticsMonth').addEventListener('change',event=>{if(event.target.value)renderReaderConfirmations(statisticsData.readers,statisticsData.assignments,event.target.value)});
load();
