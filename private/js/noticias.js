...
function showFormError(form,message){const element=document.createElement('p');clearFormMessage(form);element.className='form-message form-error';element.setAttribute('role','alert');element.setAttribute('aria-live','polite');element.textContent=message;form.querySelector('.actions').before(element)}
function render(){
  const list=$('#newsList');
  ...
  list.innerHTML=news.map(item=>{const status=statusOf(item);return `<article class="news-card ${status.key}"><div class="news-card-head"><div><span class="news-status ${status.key}">${status.label}</span><h3>${esc(item.title)}</h3></div>${isAdmin?`<div class="card-actions"><button class="small-btn edit-news" data-id="${esc(item.id)}">Editar</button><button class="small-btn danger delete-news" data-id="${esc(item.id)}">Eliminar</button></div>`:''}</div><p>${esc(item.message)}</p>${isAdmin?`<div class="news-dates"><span><b>Desde:</b> ${esc(formatDateTime(item.startsAt))}</span><span><b>Hasta:</b> ${esc(formatDateTime(item.expiresAt))}</span></div>`:''}</article>`}).join('')
}
async function load(){try{if(isAdmin){if(sessionStorage.getItem('admin_access_verified')!=='yes')return location.replace('/login.html');const auth=await request('/api/auth/status');if(!auth.authenticated)return location.replace('/login.html')}news=await request('/api/news');render()}catch(error){const dialog=$('#newsDialog');if(dialog?.open)showFormError($('#newsForm'),error.message);else toast(error.message,true)}}
function openNews(item={}){const form=$('#newsForm'),now=costaRicaDateTime(),defaultEnd=new Date(Date.now()+7*24*60*60*1000);form.reset();clearFormMessage(form);form.elements.id.value=item.id||'';form.elements.title.value=item.title||'';form.elements.message.value=item.message||'';form.elements.startsAt.value=item.startsAt||now;form.elements.expiresAt.value=item.expiresAt||`${costaRicaDateTime(defaultEnd).slice(0,10)}T23:59`;form.elements.active.checked=item.active!==false;$('#newsDialogTitle').textContent=item.id?'Editar noticia':'Nueva noticia';$('#newsDialog').showModal()}
...
$('#newsForm').addEventListener('submit',async event=>{event.preventDefault();const form=event.currentTarget,id=form.elements.id.value,payload={title:form.elements.title.value,message:form.elements.message.value,startsAt:form.elements.startsAt.value,expiresAt:form.elements.expiresAt.value,active:form.elements.active.checked};clearFormMessage(form);try{await request(`/api/news${id?`/${id}`:''}`,{method:id?'PUT':'POST',body:JSON.stringify(payload)});form.closest('dialog').close();toast('Noticia guardada');await load()}catch(error){showFormError(form,error.message)}});
load();
